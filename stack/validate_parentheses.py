...
#     idea if too loop through each element of string for each closed bracket we
#     will have the top open bracket equal to the current closed bracket.
...
#         loop through all character of string and we have empty stack. Because
#         every open have closed
...
#     why using stack
#         the idea is using stack to keep track of opening bracket. when finding
#         the closed bracket pop it. and check until we do not have anything
#         left
#         using map (or dict) to map between open and close bracket.
#         condition to return False
#             stack is empty or we have a closed bracket but the do not have top
#             of stack equal.
###################################reference solution
class Solution:
